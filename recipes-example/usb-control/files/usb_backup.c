...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(argv[1], "enable") == 0) {
        if (ActivateUSB() == 0) {
            printf("USB ports activated successfully.\n");
        } else {
            printf("Failed to activate USB ports.\n");
        }
    } else if (strcmp(argv[1], "disable") == 0) {
        if (DeactivateUSB() == 0) {
            printf("USB ports deactivated successfully.\n");
        } else {
            printf("Failed to deactivate USB ports.\n");
        }
    } else {
        fprintf(stderr, "Invalid argument: %s. Use 'enable' or 'disable'.\n", argv[1]);
        return EXIT_FAILURE;
    }
